...
list.files( path='data/', pattern='save_v1.*rds', full.names=TRUE ) %>%
  enframe(value = 'filename' ) %>%
  separate( col=filename, 
           into=c(NA, NA, qc(min_staypoint_time, max_jump_time, max_staypoint_distance, sigma)), 
           sep='_', 
           extra='drop' ,
           remove=FALSE)  %>%
  select( min_staypoint_time, max_jump_time, max_staypoint_distance, sigma, everything() ) %>%
  mutate_at(1:4, function(x) {sprintf('%07.2f', as.numeric(x) ) }) %>% 
  #head(2) %>%
  rowwise() %>%
...
df_survey_time_matches %>%
  separate( col=filename, 
           into=c(NA, qc(min_staypoint_time, max_jump_time, max_staypoint_distance, sigma, NA)), 
           sep='_', 
           remove=FALSE)  %>%
select( min_staypoint_time, max_jump_time, max_staypoint_distance, sigma, everything() ) %>%
...
df_both_matches %>%
  ggplot( aes(x=(sigma), y=both_hits/n_staypoint, fill=( sigma) )) +
  geom_boxplot()
